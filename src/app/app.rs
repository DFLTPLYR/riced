use iced::widget::{canvas, container, stack, text, Space};
...
static LAST_CURSOR_GLOBAL: std::sync::LazyLock<std::sync::Mutex<std::collections::HashMap<iced::window::Id, Point>>> =
...
static LAST_MOUSE_MOVE: LazyLock<Mutex<Instant>> =
...
fn throttled_graft(event: Event, _status: iced::event::Status, id: iced::window::Id) -> Option<Plant> {
...
impl Plots {
    ...
    fn intersects(a: &SelectionRect, screen_x: f32, screen_y: f32, screen_w: f32, screen_h: f32) -> bool {
            ...
            Some(PlotInfo::Background(output)) => {
                ...
                let (active_rect, opacity) = if self.selection_rect.selecting {
                    ...
                    if elapsed >= 150.0 {
                        ...
                        let p = elapsed / 150.0;
                        let eased = if p < 0.5 { 2.0 * p * p } else { -1.0 + (4.0 - 2.0 * p) * p };
                        (Some(fr), 1.0 - eased)
                    }
                } else {
                    (None, 0.0)
                ...
                let context_menu_overlay: Element<'_, Plant> = if let Some(cm) = &self.context_menu {
                    if cm.open {
                        ...
                        // only show if intersects this screen
                        let in_screen = lx >= 0.0 && ly >= 0.0 && lx < screen_w && ly < screen_h;
                        if in_screen {
                            container(
                                container(text("Context Menu\n(Right clicked)\nLeft drag to select").size(12).color(Color::WHITE))
                                    .padding(8)
                ...
                let bg_label = if sr.selecting {
                    format!("selecting {}x{} at {:.0},{:.0} | screen {:.0},{:.0} {}x{}", sr.width as i32, sr.height as i32, sr.x, sr.y, screen_x, screen_y, screen_w as i32, screen_h as i32)
                } else if let Some(p) = cursor {
                    format!("BG click+drag  cursor local {p:?} global {:.0},{:.0}", screen_x + p.x, screen_y + p.y)
                } else {
                    "BG click+drag  (move cursor)  | right click for menu".to_string()
                };
                let bg_view = container(text(bg_label).size(13).color(Color::from_rgba(1.0, 1.0, 1.0, 0.7)))
                    .width(Fill)
                    .height(Fill)
                    .center_x(Fill)
                    .center_y(Fill)
    ...
    pub fn update(&mut self, message: Plant) -> Command<Plant> {
        match message {
            ...
            Plant::Wayland(WayEvent::OutputInsert(output)) => {
                ...
                // sentinel Top cleanup (from new_with_boot)
            ...
            Plant::Graft(id, event) => {
                ...
                match &event {
                    ...
                    Event::Mouse(iced::mouse::Event::ButtonPressed(Button::Right)) => {
                        let pos = LAST_CURSOR_GLOBAL.lock().unwrap().get(&id).copied().unwrap_or_else(|| self.last_cursor.get(&id).copied().unwrap_or(Point::new(0.0, 0.0)));
                        let gp = self.to_global(id, pos);
                        // contextMenu.x = mouse.x; y = mouse.y; open()
                        self.context_menu = Some(ContextMenu { x: gp.x, y: gp.y, open: true });
                        println!("right click context menu at {gp:?} (local {pos:?})");
                        Command::none()
                    }
                    Event::Mouse(iced::mouse::Event::ButtonPressed(Button::Left)) => {
                        // if (contextMenu.opened) close()
